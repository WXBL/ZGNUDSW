...
- (void)setLastUpdateTime:(NSDate *)lastUpdateTime
{
    ...
    //    _lastUpdateTime = [NSDate dateWithTimeIntervalSince1970:[[BKHttpUserDefualts refreshTimeWithMenuId:[BKUserDefault menuID]] doubleValue]];
    // 更新时间
    [self updateTimeLabel];
}
	...
	switch (_state = state) {
		...
		case RefreshStateNormal:
        {
            ...
            if (oldState == RefreshStateRefreshing) {
                ...
                //                NSString *time = [NSString stringWithFormat:@"%@-%@",[BKUserDefault menuID],[BKUserDefault seriesID]];
                //                [BKHttpUserDefualts setRefreshTimeWithMenuId:time];
#ifdef NeedAudio
